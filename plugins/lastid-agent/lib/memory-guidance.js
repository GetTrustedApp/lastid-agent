...
export function memoryGuidanceLines() {
  return [
    ...
    '',
    "CURATE, don't collect — equally reflexive, and it fires BEFORE you write.",
    'Memory is curated knowledge, not an append-only log. The default flow is',
    'search → update-the-canonical → forget-the-stale:',
    '- BEFORE every write/draft, `lastid_memory_search` the topic. If a memory',
    '  already covers it, EDIT that canonical one with `lastid_memory_update`',
    '  (supersede/merge) — do NOT write a parallel near-duplicate beside it.',
    '  Memory must MOVE with reality, not accrete copies.',
    '- When you touch a topic and notice an existing memory that is now wrong or',
    '  stale, FIX it (update) or FORGET it (`lastid_memory_forget`) in the SAME',
    '  turn — never silently route around it. An ignored-but-wrong memory is the',
    '  single biggest thing that degrades retrieval for every agent sharing this',
    '  store, so correcting it is not optional cleanup, it is part of the write.',
    'Leaving parallel duplicates or a left-behind stale claim is THE failure mode',
    "— don't produce it, and if you catch yourself having done it, consolidate now.",
    '',
    'NEVER store transient TASK STATE as durable memory: status flags ("deploy',
    'pending", "FIXED"), "NEXT: …" todos, commit-hashes-as-checklist, abandoned-',
    'work notes. A durable memory is a stable fact/decision/rule that stays true',
    'until reality changes it (then you update it) — task state belongs in the',
    'task list, not here; task-tracking creep is what pollutes relevance.',
    '',
