...
function buildOperatingContext(s, credentialsBlock = '') {
  ...
  return [
    ...
    ...memoryGuidanceLines(),
    '',
    '## Reporting plugin / agent bugs',
    '',
    'If something about THIS plugin or your runtime is hurting the session —',
    'hooks firing at the wrong time, an MCP tool returning a confusing error,',
    'a permission decision that feels off, an injection that disrupts flow,',
    'broken vault unfurl, anything else where the plugin itself is the',
    'problem — that is `lastid_report_bug` territory. Notice early, surface',
    'it to the operator in plain language ("the plugin is doing X — should I',
    'file a bug?"), and if they agree, call `lastid_report_bug({summary,',
    'details})`. Reassure them: ONLY the summary + optional details + plugin',
    'version are sent — NO logs, NO files, NO identity, NO system info. The',
    'tool is CONSENT-FIRST so check before filing.',
    '',
    'REFLEX: feel a "the plugin keeps doing X and it is annoying" moment? It',
    'is almost always worth a one-line "want me to file this as a bug?" to',
    'the operator — that\'s how the plugin gets better. Don\'t wait to be',
    'asked. Don\'t file silently either; consent first.',
    '',
    '## Quick reference',
    '',
    ...
    '- `lastid_report_bug({summary, details?, email?})` — file a plugin / '
        + 'agent bug (consent-first; only summary + details + plugin version '
        + 'are sent, no logs / files / identity).',
    '',
